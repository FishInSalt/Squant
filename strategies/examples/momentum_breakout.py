"""激进型动量突破策略 — 多周期 EMA 趋势跟踪

结合手动聚合的 5 分钟级别趋势方向，在 1 分钟级别捕捉动量突破信号。
仅在 5m 趋势看多时顺势做多（现货市场不支持做空）。

适用场景：趋势行情、波动放大期。在偏空大环境中，仅捕捉反弹波段。

策略特点：
- 多时间框架：手动将 1m K 线聚合为 5m，计算 5m EMA 判断趋势方向
- ATR 动态止损/止盈：根据波动率自动调整出场位置
- 金字塔加仓：趋势确认后允许顺势加仓（最多 1 次）
- 连续亏损降仓：连续亏损 4 次后仓位减半

注意事项：
- 现货做多限制：仅在 5m 趋势为 BULLISH 时开仓。偏空市场中交易频率会降低。
- 5m 数据聚合：每 5 根 1m K 线取一次收盘价作为 5m close，非严格时间对齐。
- 移动止损在策略内模拟，非交易所止损单。
...
class MomentumBreakoutStrategy(Strategy):  # noqa: F821
    ...
    做多条件：5m看多 + 1m EMA金叉 + 价格在趋势均线上方 + MACD动量增强 + 放量
    平仓条件：ATR止损 / 保本止损 / 追踪止损 / ATR止盈
    ...
    参数:
        ...
        position_size_pct (float): 每笔仓位占总资金比例，默认 0.05
        max_daily_trades (int): 日内最大交易次数，默认 20
        pyramid_max (int): 最大加仓次数，默认 2（初始 + 1次加仓）
        stop_loss_atr_mult (float): 止损 ATR 倍数，默认 1.5
        take_profit_atr_mult (float): 止盈 ATR 倍数，默认 3.0
        ...
        daily_loss_limit (float): 日内止损上限占总资金比，默认 0.01
        loss_scale_threshold (int): 连续亏损多少次后降仓，默认 4
    """
    ...
    def on_init(self):
        ...
        # 仓位管理
        self.position_size_pct = Decimal(str(self.ctx.params.get("position_size_pct", 0.05)))
        self.max_daily_trades = self.ctx.params.get("max_daily_trades", 20)
        ...
        self.tp_atr_mult = Decimal(str(self.ctx.params.get("take_profit_atr_mult", 3.0)))
        ...
        # 风控
        self.daily_loss_limit = Decimal(str(self.ctx.params.get("daily_loss_limit", 0.01)))
        self.loss_scale_threshold = self.ctx.params.get("loss_scale_threshold", 4)

        # 运行状态（沙箱不允许 _ 前缀属性和 += 赋值属性）
        self.htf_closes = []
        self.bar_count = 0
        self.daily_trades = 0
    ...
    def on_bar(self, bar):
        ...
        if pos:
            ...
            # 2. 保本止损：盈利达 breakeven_atr 倍 ATR 后，止损移至成本价
