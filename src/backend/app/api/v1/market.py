...
async def get_candles(
    symbol: str,
    timeframe: str = Query("1h", description="时间周期：1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M"),
    ...
    result = await MarketDataFetcher.get_binance_klines(
        symbol=symbol.upper(),
        timeframe=timeframe,
        limit=limit
    )
